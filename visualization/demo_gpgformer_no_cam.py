...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--bbox-expand", type=float, default=1.4, help="Square crop expansion factor.")
    parser.add_argument("--default-focal-full", type=float, default=5000.0, help="Default full-image focal length used to solve tvec.")
    parser.add_argument(
        "--overlay-mode",
        type=str,
        default="image_space_shaded",
        choices=("image_space", "image_space_shaded", "perspective"),
        help="How to render overlays: flat image-space mapping, shaded image-space mapping, or perspective+tvec recovery.",
    )
    parser.add_argument("--tvec-mode", type=str, default="detector_refine", choices=("detector_refine", "hybrid_vertex_refine", "solve", "weakcam"), help="How to recover full-image camera translation.")
    ...
    parser.add_argument("--show-crop-box", action="store_true", help="Overlay the expanded square crop used as the actual model input.")
    parser.add_argument("--enable-container-filter", action="store_true", help="Enable filtering of abnormally large detections that cover multiple smaller hand boxes.")
    parser.add_argument("--disable-container-filter", action="store_true", help="Legacy alias. Container filtering is off by default unless --enable-container-filter is set.")
...
def _render_mesh_rgba(
    ...
    pyr_mesh = pyrender.Mesh.from_trimesh(mesh, material=material, smooth=True)
...
def render_mesh_rgba_image_space(
    ...
    color_rgb255 = tuple(int(round(c * 255.0)) for c in color_rgb)
    ...
    for face_idx in order.tolist():
        ...
        if ((poly[:, 0] < -width).all() or (poly[:, 0] > 2 * width).all() or (poly[:, 1] < -height).all() or (poly[:, 1] > 2 * height).all()):
            continue
        cv2.fillConvexPoly(rgb, poly, color_rgb255, lineType=cv2.LINE_AA)
...
def render_mesh_rgba_image_space_shaded(
    vertices_2d: np.ndarray,
    ...
    verts2d = np.asarray(vertices_2d, dtype=np.float32).reshape(-1, 2)
    verts3d = np.asarray(vertices_3d, dtype=np.float32).reshape(-1, 3)
    depths = np.asarray(vertices_depth, dtype=np.float32).reshape(-1)
    faces_arr = np.asarray(faces, dtype=np.int64).reshape(-1, 3)

    base_rgb = np.array(_as_rgb01(color_hex), dtype=np.float32)
    rgb = np.zeros((height, width, 3), dtype=np.uint8)
    alpha = np.zeros((height, width), dtype=np.uint8)

    light_dir = np.array([0.28, -0.18, 0.94], dtype=np.float32)
    ...
    for face_idx in order.tolist():
        ...
        pts = verts2d[tri]
        tri3d = verts3d[tri]
        if not np.isfinite(pts).all() or not np.isfinite(tri3d).all():
            continue
        ...
        normal = np.cross(tri3d[1] - tri3d[0], tri3d[2] - tri3d[0]).astype(np.float32)
        normal_norm = float(np.linalg.norm(normal))
        if normal_norm < 1e-8:
            continue
        normal /= normal_norm
        diffuse = abs(float(np.dot(normal, light_dir)))
        view_term = abs(float(normal[2]))
        shade = float(np.clip(0.72 + 0.20 * diffuse + 0.08 * view_term, 0.62, 0.98))
        lift = 0.10 * (1.0 - shade)
        tri_color_rgb = np.clip(base_rgb * shade + lift, 0.0, 1.0)
        tri_color_rgb255 = tuple(int(round(c * 255.0)) for c in tri_color_rgb)

        cv2.fillConvexPoly(rgb, poly, tri_color_rgb255, lineType=cv2.LINE_AA)
        cv2.fillConvexPoly(alpha, poly, 160, lineType=cv2.LINE_AA)
...
def alpha_bbox_from_rgba(rgba: np.ndarray, alpha_thresh: int = 8) -> np.ndarray | None:
...
def render_overlay_image_space_shaded(
    full_image_bgr: np.ndarray,
    vertices_2d: np.ndarray,
    vertices_3d: np.ndarray,
    vertices_depth: np.ndarray,
    ...
    h, w = full_image_bgr.shape[:2]
    rgba = render_mesh_rgba_image_space_shaded(vertices_2d, vertices_3d, vertices_depth, faces, color_hex, width=w, height=h)
    return composite_rgba_over_bgr(full_image_bgr, rgba)
...
def render_overlay_multiple_image_space_shaded(
    full_image_bgr: np.ndarray,
    vertices_2d_list: list[np.ndarray],
    vertices_3d_list: list[np.ndarray],
    vertices_depth_list: list[np.ndarray],
    faces: np.ndarray,
    color_hex: str,
) -> np.ndarray:
    h, w = full_image_bgr.shape[:2]
    rgba_all = np.zeros((h, w, 4), dtype=np.uint8)
    for vertices_2d, vertices_3d, vertices_depth in zip(vertices_2d_list, vertices_3d_list, vertices_depth_list):
        rgba = render_mesh_rgba_image_space_shaded(vertices_2d, vertices_3d, vertices_depth, faces, color_hex, width=w, height=h)
        alpha = rgba[:, :, 3:4].astype(np.float32) / 255.0
        prev_rgb = rgba_all[:, :, :3].astype(np.float32)
        prev_a = rgba_all[:, :, 3:4].astype(np.float32) / 255.0
...
def translation_fit_objective(
    vertices_3d: np.ndarray,
    vertices_2d_target: np.ndarray,
    cam_param: np.ndarray,
    cam_t: np.ndarray,
    target_bbox_xyxy: np.ndarray,
) -> float:
    proj = project_points_with_translation(vertices_3d, cam_param, cam_t)
    finite = np.isfinite(proj).all(axis=1) & np.isfinite(vertices_2d_target).all(axis=1)
    if int(finite.sum()) < 16:
        return float("inf")

    reproj_rmse = float(np.sqrt(np.mean(np.sum((proj[finite] - vertices_2d_target[finite]) ** 2, axis=1))))
    ...
    return reproj_rmse + 20.0 * center_term + 10.0 * size_term


def translation_fit_terms(
    vertices_3d: np.ndarray,
    vertices_2d_target: np.ndarray,
    cam_param: np.ndarray,
    cam_t: np.ndarray,
    ...
    center_term = float(np.linalg.norm(proj_c - target_c) / target_diag)
    size_term = abs(np.log(proj_w / target_w)) + abs(np.log(proj_h / target_h))
    return reproj_rmse, center_term, size_term


def hybrid_refine_translation_with_vertices(
    vertices_3d: np.ndarray,
    vertices_2d_target: np.ndarray,
    cam_param: np.ndarray,
    init_cam_t: np.ndarray,
    ...
    init = np.asarray(init_cam_t, dtype=np.float32).reshape(3)
    detector_cam_t = refine_translation_to_bbox(vertices_3d, cam_param, init, target_bbox_xyxy, num_iters=4)
    best = detector_cam_t.astype(np.float32)
    base_terms = translation_fit_terms(vertices_3d, vertices_2d_target, cam_param, detector_cam_t, target_bbox_xyxy)
    if base_terms is None:
        return best

    base_reproj, base_center, base_size = base_terms
    best_score = base_reproj + 28.0 * base_center + 18.0 * base_size

    vertex_cam_t = solve_translation_from_2d_correspondences(vertices_3d, vertices_2d_target, cam_param)
    if vertex_cam_t is None or not np.isfinite(vertex_cam_t).all() or float(vertex_cam_t[2]) <= 1e-4:
        return best

    conservative = detector_cam_t.astype(np.float32).copy()
    conservative[0] = 0.75 * detector_cam_t[0] + 0.25 * float(vertex_cam_t[0])
    conservative[1] = 0.75 * detector_cam_t[1] + 0.25 * float(vertex_cam_t[1])
    z_ratio = float(vertex_cam_t[2]) / max(float(detector_cam_t[2]), 1e-6)
    z_ratio = float(np.clip(z_ratio, 0.95, 1.05))
    conservative[2] = float(detector_cam_t[2]) * z_ratio
    conservative = refine_translation_to_bbox(vertices_3d, cam_param, conservative, target_bbox_xyxy, num_iters=1)

    cand_terms = translation_fit_terms(vertices_3d, vertices_2d_target, cam_param, conservative, target_bbox_xyxy)
    if cand_terms is None:
        return best

    cand_reproj, cand_center, cand_size = cand_terms
    z_reg = abs(np.log(float(conservative[2]) / max(float(detector_cam_t[2]), 1e-6)))
    cand_score = cand_reproj + 28.0 * cand_center + 18.0 * cand_size + 35.0 * z_reg

    if cand_size <= base_size + 0.05 and cand_center <= base_center + 0.05 and cand_score < best_score:
        best = conservative.astype(np.float32)
...
def bbox_intersection_area_xyxy(box_a: np.ndarray, box_b: np.ndarray) -> float:
...
def filter_large_container_detections(
    detections: list[dict[str, Any]],
    image_shape_hw: tuple[int, int],
    large_area_ratio: float = 0.30,
    contains_ratio: float = 0.90,
    huge_area_ratio: float = 0.45,
    max_small_area_ratio: float = 0.60,
    strong_overlap_ratio: float = 0.35,
    large_overlap_area_ratio: float = 0.35,
) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    if len(detections) <= 1:
        return detections, []

    img_h, img_w = [int(v) for v in image_shape_hw]
    img_area = max(float(img_h * img_w), 1.0)
    kept: list[dict[str, Any]] = []
    removed: list[dict[str, Any]] = []

    areas = [bbox_area_xyxy(det["bbox_xyxy"]) for det in detections]
    for idx, det in enumerate(detections):
        area_i = areas[idx]
        area_ratio = area_i / img_area
        contains_count = 0
        ...
        for jdx, other in enumerate(detections):
            if idx == jdx:
                continue
            area_j = areas[jdx]
            if area_j <= 1e-6 or area_j >= area_i * max_small_area_ratio:
                continue
            inter = bbox_intersection_area_xyxy(det["bbox_xyxy"], other["bbox_xyxy"])
            overlap_ratio = inter / area_j
            if overlap_ratio >= contains_ratio:
                contains_count += 1
            if area_ratio >= large_overlap_area_ratio and overlap_ratio >= strong_overlap_ratio:
                strong_overlap_count += 1

        should_remove = (
            (area_ratio >= large_area_ratio and contains_count >= 2)
            or (area_ratio >= huge_area_ratio and contains_count >= 1)
            or (area_ratio >= large_overlap_area_ratio and strong_overlap_count >= 3)
        )
        if should_remove:
            det_removed = dict(det)
            det_removed["filtered_reason"] = {
                "type": "large_container_bbox",
                "area_ratio": float(area_ratio),
                "contains_count": int(contains_count),
                "strong_overlap_count": int(strong_overlap_count),
            }
            removed.append(det_removed)
        else:
            kept.append(det)
...
def infer_one_hand(
    ...
    elif tvec_mode == "hybrid_vertex_refine":
        pred_cam_t_full = hybrid_refine_translation_with_vertices(
            vertices_3d=pred_vertices,
            vertices_2d_target=pred_vertices_2d_full,
...
def main() -> None:
    ...
    if args.overlay_mode in ("image_space", "image_space_shaded"):
        print(f"[demo_gpgformer_no_cam] overlay_mode={args.overlay_mode}, so tvec_mode/default_focal_full do not affect overlay rendering.")
    ...
    for image_path in image_paths:
        ...
        use_container_filter = bool(args.enable_container_filter and not args.disable_container_filter)
        if detections and use_container_filter:
            detections, filtered_out_detections = filter_large_container_detections(detections, image_bgr.shape[:2])
        ...
        image_result: dict[str, Any] = {
            ...
            "overlay_mode": args.overlay_mode,
            "container_filter_enabled": use_container_filter,
            "num_filtered_detections": len(filtered_out_detections),
            "filtered_detections": [
                {
                    "bbox_xyxy": det["bbox_xyxy"].tolist(),
                    "score": det["score"],
                    "is_right": det["is_right"],
                    "filtered_reason": det.get("filtered_reason"),
                }
                for det in filtered_out_detections
            ],
            "detections": [],
        }
        all_vertices_2d: list[np.ndarray] = []
        all_vertices_3d: list[np.ndarray] = []
        ...
        for det_idx, det in enumerate(detections):
            ...
            elif args.overlay_mode == "image_space_shaded":
        ...
        elif args.overlay_mode == "image_space_shaded" and all_vertices_2d:
