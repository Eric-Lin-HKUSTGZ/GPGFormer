...
from visualization.compare_hand_mesh import render_mesh_only, render_overlay

MESH_RENDER_BACKEND = "pyrender"
...
DEFAULT_RGB_CKPTS = {
    "ho3d": "/root/code/vepfs/GPGFormer/checkpoints/ablations_v2/mixed_ho3d_rgb_only_20260403/ho3d/gpgformer_best.pt",
    "freihand": "/root/code/vepfs/GPGFormer/checkpoints/freihand_20260304_rgb_only/freihand/gpgformer_best.pt",
}
DEFAULT_OUTPUT_DIRS = {
    "ho3d": str(REPO_ROOT / "outputs" / "ho3d_feature_mesh_compare_20260404"),
    "freihand": str(REPO_ROOT / "outputs" / "freihand_feature_mesh_compare_20260404"),
}
MESH_COLOR_HEX = {
    "gt": "#5C5C5C",
    "rgb_only": "#2A9D8F",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Visualize HO3D or FreiHAND feature heatmaps and reconstructed meshes for RGB-only vs multimodal GPGFormer checkpoints."
        )
    )
    parser.add_argument("--dataset", choices=["ho3d", "freihand"], default="ho3d")
    parser.add_argument("--checkpoint", type=str, default=None)
    parser.add_argument("--rgb-checkpoint", type=str, default=None)
    parser.add_argument("--output-dir", type=str, default=None)
    parser.add_argument("--device", type=str, default="cuda" if torch.cuda.is_available() else "cpu")
    parser.add_argument("--split", choices=["train", "eval", "test", "val", "evaluation"], default="evaluation")
    ...
    parser.add_argument("--mesh-render-size", type=int, default=512)
    parser.add_argument("--mesh-view-azim", type=float, default=35.0)
    parser.add_argument("--mesh-view-elev", type=float, default=-20.0)
    parser.add_argument(
        "--show-bbox",
        action="store_true",
...
def resolve_dataset_split(dataset_name: str, split: str) -> str:
    ...
    if dataset_name == "ho3d":
        return "train" if split == "train" else "evaluation"
    if dataset_name == "freihand":
        return "train" if split == "train" else "eval"
    raise ValueError(f"Unsupported dataset: {dataset_name}")


def select_sample_indices(dataset: Any, args: argparse.Namespace, resolved_split: str) -> List[int]:
    if args.sample_indices:
        return [int(idx) for idx in args.sample_indices]
    total = len(dataset)
...
def build_dataset(dataset_name: str, cfg: dict, split: str) -> Any:
    if dataset_name == "ho3d":
        return build_ho3d_dataset(cfg, split=split)
    if dataset_name == "freihand":
        return build_freihand_dataset(cfg, split=split)
    raise ValueError(f"Unsupported dataset: {dataset_name}")
...
def build_model_from_cfg(cfg: dict, device: torch.device) -> GPGFormer:
    ...
    fusion_proj_init_mode = str(model_cfg.get("fusion_proj_init_mode", "tiny")).strip().lower()
    if fusion_proj_init_mode == "tiny":
        fusion_proj_init_mode = "small"
    model = GPGFormer(
        GPGFormerConfig(
            ...
            fusion_proj_init_mode=fusion_proj_init_mode,
...
def forward_with_geometry(
    ...
    enc_inputs = prepare_encoder_inputs(model, img_01, cam_param=cam_param, use_geometry=use_geometry)
    ...
    if model.feature_refiner is not None and model.feature_refiner.is_feature_enhancer:
        img_feat = model.feature_refiner.enhance_features(img_feat)

    mano_params, pred_cam = model.mano_head(img_feat)

    B = img_feat.shape[0]
    focal_length_px = enc_inputs["focal_length_px"]
    if focal_length_px is None:
        focal = torch.full((B, 2), float(model.cfg.focal_length), device=img_feat.device, dtype=img_feat.dtype)
    else:
        ...
        if fl.ndim == 0:
            ...
            raise ValueError(f"Unsupported focal_length_px shape: {tuple(fl.shape)}")

    kcr_aux: Optional[Dict[str, torch.Tensor]] = None
    if model.feature_refiner is not None and not model.feature_refiner.is_feature_enhancer:
        num_steps = 1
        if model.feature_refiner.method == "sjta":
            num_steps = int(getattr(model.feature_refiner.cfg, "sjta_num_steps", 1))
            num_steps = max(1, num_steps)

        for _ in range(num_steps):
            with torch.no_grad():
                init_mano_out = model.mano(mano_params, pose2rot=False)
                if model.feature_refiner.method in ("sjta", "kcr"):
                    ...
                    init_joints = model._kp21_from_verts(init_mano_out.vertices, J_reg)
                else:
                    init_joints = vertices2joints(model.mano.mano.J_regressor, init_mano_out.vertices)
            ...
            elif model.feature_refiner.method == "kcr":
                pose_rm = torch.cat([mano_params["global_orient"], mano_params["hand_pose"]], dim=1)
                pose_aa = model._rotmat_to_aa(pose_rm.view(-1, 3, 3)).view(B, -1)
            ...
            refined_params, pred_cam = model.feature_refiner.refine_params(
                img_feat=img_feat,
                joints_3d=init_joints,
                pred_cam=pred_cam,
                pred_mano_feats=pred_mano_feats,
                focal_length=focal,
                img_size=float(model.cfg.image_size),
            )
            if model.feature_refiner.method == "kcr":
                kcr_aux = getattr(model.feature_refiner, "last_aux", None)
    ...
    pred_cam_t = torch.stack(
        [
            pred_cam[:, 1],
            pred_cam[:, 2],
            2.0 * focal[:, 0] / (model.cfg.image_size * pred_cam[:, 0] + 1e-9),
        ],
        dim=-1,
    )

    if model.mano_decoder == "freihand_legacy":
        model._init_freihand_mano_layer(device=img_feat.device, dtype=img_feat.dtype)
        pose_rm = torch.cat([mano_params["global_orient"], mano_params["hand_pose"]], dim=1)
        pose_aa = model._rotmat_to_aa(pose_rm.view(-1, 3, 3)).view(B, 16, 3).reshape(B, 48)
        betas = mano_params["betas"]
        trans = torch.zeros((B, 3), device=img_feat.device, dtype=img_feat.dtype)
        verts_mm, _ = model._freihand_mano_layer(pose_aa, betas, trans)
        J_reg = getattr(model._freihand_mano_layer, "th_J_regressor", None)
        if J_reg is None:
            raise AttributeError("freihand MANO layer missing th_J_regressor")
        kp21_mm = model._freihand_kp21_from_verts_mm(verts_mm, J_reg)
        v_cam = verts_mm / 1000.0
        j_cam = kp21_mm / 1000.0
        pred_keypoints_3d_raw = None
    else:
        ...
        j_cam = model._kp21_from_verts(v_cam, J_reg)
        pred_keypoints_3d_raw = mano_out.joints
    ...
    out = {
        ...
        "pred_keypoints_3d_raw": pred_keypoints_3d_raw,
        "used_geometry": bool(use_geometry),
    }
...
def compute_rollout_maps(
    attns: Sequence[torch.Tensor], layer_numbers: Sequence[int], hp: int, wp: int
) -> Dict[int, np.ndarray]:
    rollout = None
...
def keypoints_to_pixels(
    sample: Dict[str, Any], joint_names: Sequence[str], img_h: int, img_w: int
) -> Dict[str, np.ndarray]:
...
def project_predicted_joints(
    joints_3d: torch.Tensor, pred_cam: torch.Tensor, focal_length: torch.Tensor, img_h: int, img_w: int
) -> np.ndarray:
...
def to_bgr_u8(image_rgb: np.ndarray) -> np.ndarray:
    return cv2.cvtColor(np.uint8(np.clip(image_rgb, 0.0, 1.0) * 255.0), cv2.COLOR_RGB2BGR)


def to_rgb01(image_bgr: np.ndarray) -> np.ndarray:
    return cv2.cvtColor(np.asarray(image_bgr, dtype=np.uint8), cv2.COLOR_BGR2RGB).astype(np.float32) / 255.0


def render_prediction_mesh_assets(
    bundle: FeatureDebugBundle,
    render_size: int,
    mesh_view_azim: float,
    mesh_view_elev: float,
) -> tuple[np.ndarray, np.ndarray]:
    image_bgr = to_bgr_u8(bundle.input_image)
    overlay_bgr = render_overlay(
        image_bgr,
        bundle.pred_vertices,
        bundle.faces,
        bundle.pred_cam_t,
        bundle.crop_cam_param,
        MESH_COLOR_HEX[bundle.model_name],
        bundle.is_right,
    )
    mesh_bgr = render_mesh_only(
        bundle.pred_vertices,
        bundle.faces,
        MESH_COLOR_HEX[bundle.model_name],
        bundle.is_right,
        int(render_size),
        view_azim_deg=float(mesh_view_azim),
        view_elev_deg=float(mesh_view_elev),
    )
    return overlay_bgr, mesh_bgr


def render_gt_mesh_assets(
    sample: Dict[str, Any],
    ...
    vertices_gt = sample.get("vertices_gt", None)
    if vertices_gt is None:
        return None
    vertices_gt_np = as_numpy(vertices_gt).astype(np.float32)
    cam_param = as_numpy(sample["cam_param"]).astype(np.float32)
    is_right = bool(float(sample.get("is_right", 1.0)) > 0.5)
    image_bgr = to_bgr_u8(image_rgb)
    overlay_bgr = render_overlay(
        image_bgr,
        vertices_gt_np,
        faces,
...
def save_mesh_overview(
    path: Path,
    bundles: Sequence[FeatureDebugBundle],
    gt_overlay_bgr: Optional[np.ndarray],
    gt_mesh_bgr: Optional[np.ndarray],
    pred_mesh_assets: Dict[str, tuple[np.ndarray, np.ndarray]],
) -> None:
    include_gt = gt_overlay_bgr is not None and gt_mesh_bgr is not None
    cols = len(bundles) + (1 if include_gt else 0)
    fig, axes = plt.subplots(2, cols, figsize=(5.2 * cols, 8.0))
    if cols == 1:
        axes = np.expand_dims(axes, axis=1)

    col = 0
    if include_gt:
        axes[0, col].imshow(to_rgb01(gt_overlay_bgr))
        axes[0, col].set_title("gt | mesh overlay")
        axes[0, col].axis("off")
        axes[1, col].imshow(to_rgb01(gt_mesh_bgr))
        axes[1, col].set_title("gt | mesh only")
        axes[1, col].axis("off")
        col += 1

    for bundle in bundles:
        overlay_bgr, mesh_bgr = pred_mesh_assets[bundle.model_name]
        axes[0, col].imshow(to_rgb01(overlay_bgr))
        axes[0, col].set_title(f"{bundle.model_name} | mesh overlay")
        axes[0, col].axis("off")
        axes[1, col].imshow(to_rgb01(mesh_bgr))
        axes[1, col].set_title(f"{bundle.model_name} | mesh only")
        axes[1, col].axis("off")
        col += 1
...
def build_bundle(
    ...
    with torch.no_grad():
        out = forward_with_geometry(model, img_01=img, cam_param=cam_param, use_geometry=use_geometry)
    ...
    gt_keypoints_crop_all = np.stack(
        [
            (as_numpy(sample["keypoints_2d"])[:, 0] + 0.5) * float(img_w),
            (as_numpy(sample["keypoints_2d"])[:, 1] + 0.5) * float(img_h),
        ],
        axis=-1,
    ).astype(np.float32)
    uv_valid = as_numpy(sample.get("uv_valid", np.ones((gt_keypoints_crop_all.shape[0],), dtype=np.float32))).astype(
        np.float32
    )
    pred_keypoints_3d = out["pred_keypoints_3d"][0].detach().cpu().numpy().astype(np.float32)
    pred_cam_t_solved = solve_translation_from_2d_correspondences(
        pred_keypoints_3d,
        gt_keypoints_crop_all,
        as_numpy(sample["cam_param"]).astype(np.float32),
        valid=uv_valid,
    )
    pred_cam_t = out["pred_cam_t"][0].detach().cpu().numpy().astype(np.float32)
    camera_translation_source = "model_pred_cam_t"
    if pred_cam_t_solved is not None:
        pred_cam_t = pred_cam_t_solved.astype(np.float32)
        camera_translation_source = "solve_translation_from_2d_correspondences"

    metrics: Dict[str, Any] = {
        "uses_geometry": bool(use_geometry),
        "img_feat": {"mass_in_hand_bbox": heatmap_mass_in_bbox(upsample_to_image(img_feat_map, img_h, img_w), hand_bbox)},
        "rollout": {},
        "sjta": {},
        "mesh": {"camera_translation_source": camera_translation_source},
    }
    for layer in layer_numbers:
    ...
    for joint_name in joint_names:
        ...
        metrics["sjta"][joint_name] = {
            ...
            "peak_to_gt_px": float(
                np.linalg.norm(np.array([peak_x, peak_y], dtype=np.float32) - gt_xy.astype(np.float32))
            ),
            "pred_to_gt_px": float(
                np.linalg.norm(pred_joint_pixels[joint_name].astype(np.float32) - gt_xy.astype(np.float32))
            ),
        }
    ...
    return FeatureDebugBundle(
        ...
        pred_cam_t=pred_cam_t,
        faces=np.asarray(model.mano.mano.faces, dtype=np.int64),
        crop_cam_param=as_numpy(sample["cam_param"]).astype(np.float32),
        is_right=bool(float(as_numpy(sample.get("is_right", 1.0)).reshape(-1)[0]) > 0.5),
        metrics=metrics,
    )


def resolve_sample_identifier(dataset_name: str, sample_idx: int, sample: Dict[str, Any], dataset: Any) -> Tuple[str, str]:
    seq_name = str(sample.get("seq_name", "")).strip()
    frame_id = str(sample.get("frame_id", "")).strip()
    if seq_name and frame_id:
        identifier = f"{seq_name}/{frame_id}"
        slug = f"sample_{sample_idx:05d}_{sanitize_name(seq_name)}_{sanitize_name(frame_id)}"
        return identifier, slug
    if dataset_name == "freihand":
        real_idx = int(getattr(dataset, "indices", list(range(len(dataset))))[sample_idx])
        return f"img_{real_idx:08d}", f"sample_{sample_idx:05d}_img_{real_idx:08d}"
    return f"idx_{sample_idx:05d}", f"sample_{sample_idx:05d}"
...
def save_sample_outputs(
    ...
    reducer_name: str,
    show_bbox: bool,
    mesh_render_size: int,
    mesh_view_azim: float,
    mesh_view_elev: float,
    sample_real_index: Optional[int] = None,
) -> Dict[str, Any]:
    ...
    gt_mesh_assets = render_gt_mesh_assets(
        sample=sample,
        image_rgb=image_rgb,
        faces=bundles[0].faces,
        render_size=mesh_render_size,
        mesh_view_azim=mesh_view_azim,
        mesh_view_elev=mesh_view_elev,
    )
    pred_mesh_assets: Dict[str, tuple[np.ndarray, np.ndarray]] = {}
    for bundle in bundles:
        pred_mesh_assets[bundle.model_name] = render_prediction_mesh_assets(
            bundle=bundle,
            render_size=mesh_render_size,
            mesh_view_azim=mesh_view_azim,
            mesh_view_elev=mesh_view_elev,
    ...
    metadata: Dict[str, Any] = {
        ...
        "img_path": str(sample.get("img_path", "")),
        "mesh_render_backend": MESH_RENDER_BACKEND,
        "layers": [int(x) for x in layer_numbers],
        "joint_names": list(joint_names),
        "img_feat_reducer": reducer_name,
        "show_bbox": bool(show_bbox),
        "mesh_render_size": int(mesh_render_size),
        "mesh_view_azim": float(mesh_view_azim),
        "mesh_view_elev": float(mesh_view_elev),
        "files": {
            "input_crop": "input_crop.png",
            "overview_heatmap": "overview.png",
            "overview_mesh": "overview_mesh.png",
        },
        "models": {},
    }
    if sample_real_index is not None:
        metadata["real_index"] = int(sample_real_index)
    if gt_mesh_assets is not None:
        gt_overlay_bgr, gt_mesh_bgr = gt_mesh_assets
        cv2.imwrite(str(sample_dir / "gt_mesh_overlay.png"), gt_overlay_bgr)
        cv2.imwrite(str(sample_dir / "gt_mesh.png"), gt_mesh_bgr)
        metadata["files"]["gt_mesh_overlay"] = "gt_mesh_overlay.png"
        metadata["files"]["gt_mesh"] = "gt_mesh.png"
    ...
    for bundle in bundles:
        ...
        overlay_bgr, mesh_bgr = pred_mesh_assets[bundle.model_name]
        cv2.imwrite(str(model_dir / "mesh_overlay.png"), overlay_bgr)
        cv2.imwrite(str(model_dir / "mesh.png"), mesh_bgr)
        metadata["models"][bundle.model_name] = {
            **bundle.metrics,
            "pred_cam_t": [float(x) for x in bundle.pred_cam_t.tolist()],
            "mesh_files": {
                "mesh_overlay": f"{bundle.model_name}/mesh_overlay.png",
                "mesh": f"{bundle.model_name}/mesh.png",
            },
        }
    ...
    save_mesh_overview(
        path=sample_dir / "overview_mesh.png",
        bundles=bundles,
        gt_overlay_bgr=gt_mesh_assets[0] if gt_mesh_assets is not None else None,
        gt_mesh_bgr=gt_mesh_assets[1] if gt_mesh_assets is not None else None,
        pred_mesh_assets=pred_mesh_assets,
    )
...
def main() -> None:
    ...
    multimodal_ckpt = args.checkpoint or DEFAULT_MULTIMODAL_CKPTS[dataset_name]
    rgb_ckpt = args.rgb_checkpoint or DEFAULT_RGB_CKPTS[dataset_name]
    output_dir = Path(args.output_dir or DEFAULT_OUTPUT_DIRS[dataset_name])
    output_dir.mkdir(parents=True, exist_ok=True)
    device = torch.device(args.device)

    print(f"[info] dataset: {dataset_name}")
    print(f"[info] loading multimodal checkpoint: {multimodal_ckpt}")
    multimodal_model, cfg = load_model_bundle(multimodal_ckpt, device=device)
    if not bool(getattr(multimodal_model, "use_geo_prior", False)):
        raise ValueError("Checkpoint cfg has use_geo_prior=False; cannot build multimodal vs RGB-only comparison.")
    print(f"[info] loading RGB-only checkpoint: {rgb_ckpt}")
    rgb_model, _ = load_model_bundle(rgb_ckpt, device=device)

    dataset_split = resolve_dataset_split(dataset_name, args.split)
    dataset = build_dataset(dataset_name, cfg, split=dataset_split)
    sample_indices = select_sample_indices(dataset, args, dataset_split)
    total = len(dataset)
    invalid_indices = [idx for idx in sample_indices if idx < 0 or idx >= total]
    if invalid_indices:
        raise IndexError(
            f"Sample indices out of range for dataset={dataset_name} split={dataset_split} len={total}: {invalid_indices}"
        )
    ...
    summary = {
        ...
        "multimodal_checkpoint": multimodal_ckpt,
        ...
        "mesh_render_size": int(args.mesh_render_size),
        "mesh_view_azim": float(args.mesh_view_azim),
        "mesh_view_elev": float(args.mesh_view_elev),
        "rgb_only_mode": "standalone_rgb_only_checkpoint",
        "samples": [],
    }

    for sample_idx in sample_indices:
        sample = dataset[sample_idx]
        sample_identifier, sample_slug = resolve_sample_identifier(dataset_name, sample_idx, sample, dataset)
        sample_dir = output_dir / sample_slug
        sample_dir.mkdir(parents=True, exist_ok=True)
        print(f"[progress] sample_idx={sample_idx} id={sample_identifier} -> {sample_dir}")
        sample_real_index = None
        if dataset_name == "freihand":
            sample_real_index = int(getattr(dataset, "indices", list(range(len(dataset))))[sample_idx])
        ...
        summary["samples"].append(
            save_sample_outputs(
                ...
                mesh_view_azim=float(args.mesh_view_azim),
                mesh_view_elev=float(args.mesh_view_elev),
                sample_real_index=sample_real_index,
            )
        )
