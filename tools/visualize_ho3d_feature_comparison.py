...
DEFAULT_RGB_CKPT = (
    "/root/code/vepfs/GPGFormer/checkpoints/ablations_v2/mixed_ho3d_rgb_only_20260403/ho3d/gpgformer_best.pt"
)
JOINT_NAME_TO_INDEX = {
    "wrist": 0,
    "thumb_mcp": 1,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Visualize HO3D feature heatmaps for a multimodal GPGFormer checkpoint, "
            "comparing RGB-only inference (geometry disabled) against multimodal inference."
        )
    )
    parser.add_argument("--checkpoint", type=str, default=DEFAULT_MULTIMODAL_CKPT)
    parser.add_argument("--rgb-checkpoint", type=str, default=DEFAULT_RGB_CKPT)
    parser.add_argument(
        "--output-dir",
        type=str,
        default=str(REPO_ROOT / "outputs" / "ho3d_feature_compare_20260402"),
    )
...
def resolve_ho3d_split(split: str) -> str:
...
def select_sample_indices(dataset: HO3DDataset, args: argparse.Namespace) -> List[int]:
    if args.sample_indices:
        return [int(idx) for idx in args.sample_indices]
    total = len(dataset)
    count = min(max(int(args.num_samples), 0), total)
    if resolve_ho3d_split(args.split) != "train":
...
def resolve_sample_identifier(sample_idx: int, sample: Dict[str, Any]) -> Tuple[str, str]:
...
def main() -> None:
    ...
    print(f"[info] loading multimodal checkpoint: {args.checkpoint}")
    multimodal_model, cfg = load_model_bundle(args.checkpoint, device=device)
    ...
    rgb_model, _ = load_model_bundle(args.rgb_checkpoint, device=device)

    dataset_split = resolve_ho3d_split(args.split)
    dataset = build_ho3d_dataset(cfg, split=dataset_split)
    sample_indices = select_sample_indices(dataset, args)
    total = len(dataset)
    invalid_indices = [idx for idx in sample_indices if idx < 0 or idx >= total]
    if invalid_indices:
        raise IndexError(f"Sample indices out of range for HO3D split={dataset_split} len={total}: {invalid_indices}")
    ...
    for sample_idx in sample_indices:
        ...
        sample_identifier, sample_slug = resolve_sample_identifier(sample_idx, sample)
