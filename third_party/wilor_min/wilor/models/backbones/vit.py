...
# ---- Small rotation conversion helpers (avoid extra deps) ----
def rotmat_to_rot6d(R: torch.Tensor) -> torch.Tensor:
    """
    Inverse of rot6d_to_rotmat for initialization purposes.
    Args:
        R: (...,3,3)
    Returns:
        (...,6) formed by the first two columns of R.
    """
    r = R[..., :, :2]  # (...,3,2)
    r = r.transpose(-2, -1).contiguous()  # (...,2,3)
    return r.view(*R.shape[:-2], 6)


def rotmat_to_aa(R: torch.Tensor, eps: float = 1e-6) -> torch.Tensor:
    """
...
# ---- Minimal replacements for timm.models.layers ----
def to_2tuple(x):
...
def trunc_normal_(tensor, mean=0.0, std=1.0, a=-2.0, b=2.0):
    ...
    Minimal implementation (sufficient for ViT init).
    """
    # Method based on https://people.sc.fsu.edu/~jburkardt/presentations/truncated_normal.pdf
    def norm_cdf(x):
        return (1.0 + torch.erf(x / (2.0**0.5))) / 2.0
...
class ViT(nn.Module):
    ...
    def __init__(self,
        ...
        init_hand_pose_np = mean_params['pose'].astype(np.float32)
        # Make mean pose compatible with selected joint representation.
        # WiLoR ckpt/mean-params are often 6D; UniHandFormer uses axis-angle.
        if self.joint_rep_type == 'aa' and init_hand_pose_np.size == 6 * (cfg.MANO.NUM_HAND_JOINTS + 1):
            with torch.no_grad():
                pose6d = torch.from_numpy(init_hand_pose_np).view(-1, 6)
                R = rot6d_to_rotmat(pose6d)
                aa = rotmat_to_aa(R).view(-1).cpu().numpy().astype(np.float32)
            init_hand_pose_np = aa
        elif self.joint_rep_type == '6d' and init_hand_pose_np.size == 3 * (cfg.MANO.NUM_HAND_JOINTS + 1):
            with torch.no_grad():
                poseaa = torch.from_numpy(init_hand_pose_np).view(-1, 3)
                R = aa_to_rotmat(poseaa)
                pose6d = rotmat_to_rot6d(R).view(-1).cpu().numpy().astype(np.float32)
            init_hand_pose_np = pose6d

        init_hand_pose = torch.from_numpy(init_hand_pose_np).unsqueeze(0)
        ...
        # Pose decoder output matches joint representation dimensionality.
        self.decpose  = nn.Linear(self.num_features, self.joint_rep_dim)
